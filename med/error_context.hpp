...
inline char const* toString(error_context const& ec)
{
	...
	case error::OVERFLOW:
		if (ec.m_param[1])
		{
			std::snprintf(sz, sizeof(sz), "%zu bits left, '%.32s' needs %zu", ec.m_param[0], ec.m_name, ec.m_param[1]);
		}
		else
		{
			std::snprintf(sz, sizeof(sz), "%zu bits left after '%.32s'", ec.m_param[0], ec.m_name);
		}
		break;
	...
	case error::MISSING_IE:
		std::snprintf(sz, sizeof(sz), "Missing IE '%.32s': at least %zu expected, got %zu", ec.m_name, ec.m_param[0], ec.m_param[1]);
		break;

	case error::EXTRA_IE:
		std::snprintf(sz, sizeof(sz), "Excessive IE '%.32s': no more than %zu expected, got %zu", ec.m_name, ec.m_param[0], ec.m_param[1]);
		break;

	case error::OUT_OF_MEMORY:
		std::snprintf(sz, sizeof(sz), "No space to allocate IE '%.32s': %zu bytes", ec.m_name, ec.m_param[0]);
		break;
	}
