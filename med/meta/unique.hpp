...
template <class T1, class... T> requires (!std::same_as<std::nullptr_t, T1>)
struct all_ne<T1, T...>
{
	template <class T0>
	static constexpr bool apply(T0 base, T1 val1, T... vals)
	{
		return base != static_cast<T0>(val1) && all_ne<T...>::apply(base, vals...);
	}
};

template <class... T>
struct all_ne<std::nullptr_t, T...>
{
	template <class T0>
	static constexpr bool apply(T0 base, std::nullptr_t, T... vals)
	{
		return all_ne<T...>::apply(base, vals...);
	}
};
...
template <class T1, class... T> requires (!std::same_as<std::nullptr_t, T1>)
struct unique_values<T1, T...>
{
	static constexpr bool apply(T1 val, T... vals)
	{
		return all_ne<T...>::apply(val, vals...) && unique_values<T...>::apply(vals...);
	}
};
...
struct unique_values<T1, T...>
{
	...
	{
		return unique_values<T...>::apply(vals...);
	}
};
...
{
	return detail::unique_values<T...>::apply(vals...);
}
